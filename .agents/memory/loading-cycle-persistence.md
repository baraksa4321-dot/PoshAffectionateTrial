...
**How to apply:** Read and write browser storage only in a client effect, use a new non-conflicting storage key when replacing an older random scheme, and reset transient rotation when a bfcache page is restored and treated as a new opening. The loading presentation must also use a persisted authenticated gender until logout; use auth metadata as an early signal and clear it on every sign-out path so female animation/message and male spinner modes never flash into each other.