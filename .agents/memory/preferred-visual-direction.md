...
The preferred direction is a polished mobile fitness app, not a website: use a white/ivory canvas, near-black type, restrained palette accents, and a deep version of the active palette for the workout action surface. Default men to black and women to rose-gold, while preserving an explicit palette choice made before login. Keep the existing layout/order and product flows while making the shell and surfaces feel native, smooth, and finished.
