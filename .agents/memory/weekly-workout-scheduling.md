...
Explicit workout weekdays use a Sunday-first value from 0 through 6. Weekly completion is derived from sessions inside the current Sunday-to-Saturday window, so history is retained while the visible completion state resets naturally each week. Local active-session drafts, feedback, and rest timers must also be keyed by the week start.
