...
const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
  email: required("GYMTRACK_IMPORT_COACH_EMAIL"),
  password: required("GYMTRACK_IMPORT_COACH_PASSWORD"),
});
...
console.log(
  JSON.stringify(
    {
      authenticatedUser: { id: coach.id, fullName: coach.full_name, role: coach.role },
