...
| --- | --- | --- | --- |
| Sign-up, sign-in, sign-out, persisted Supabase session | PARTIAL | App shell calls Supabase sign-up/sign-in/sign-out; store listens for auth state and restores a session. Password reset, account recovery, and expired-session UX are absent. | High |
...
| Programs and days | PARTIAL | Local CRUD, ordering, builder, and Supabase upserts exist. Cloud deletion/conflict handling is absent, so removed content can return after sync. | High |
...
| Custom foods and favorites | PARTIAL | Local CRUD/favorites and cloud upserts exist. Removed custom foods/favorites are not deleted remotely. | High |
...
2. Implement password-reset/recovery and clear expired-session behavior.
3. Add remote delete/conflict behavior for every synced record type.
