...
function VideoFeedbackInbox({
  ...
  return (
    <section className="surface-card max-h-[min(82vh,52rem)] space-y-3 overflow-y-auto rounded-3xl border border-primary/20 bg-primary/[0.025] p-3 sm:p-5">
      ...
      {visibleVideos.length === 0 && visibleDemoVideos.length === 0 ? (
        ...
        <div className="space-y-4">
          {visibleVideos.length > 0 ? (
            <div className="space-y-2.5">
              <p className="px-1 text-[10px] font-bold tracking-[0.14em] text-primary uppercase">
                סרטוני ביצוע · משוב למתאמנים
              </p>
          ...
          {visibleDemoVideos.length > 0 ? (
            <div className="space-y-2.5">
              <p className="px-1 text-[10px] font-bold tracking-[0.14em] text-primary uppercase">
                סרטוני הדגמה שנוספו לתוכניות
              </p>
              {visibleDemoVideos.map((video) => (
                <article
                  key={video.id}
                  className="rounded-2xl border border-primary/15 bg-background p-2.5 sm:p-3"
                >
                  <div className="min-w-0">
                    <h3 className="truncate text-sm font-extrabold text-ink">{video.clientName}</h3>
                    <p className="mt-0.5 truncate text-[11px] font-semibold text-primary">
                      {video.workoutName} · {video.exerciseName}
                    </p>
                  </div>
                  <div className="mt-2 aspect-video overflow-hidden rounded-xl bg-black">
                    <WorkoutVideoPlayer
                      source={video.source}
                      title={`סרטון הדגמה עבור ${video.exerciseName}`}
                      className="h-full w-full rounded-xl bg-black object-contain"
                    />
                  </div>
                  <p className="mt-2 rounded-lg bg-primary/5 px-2.5 py-1.5 text-[10px] font-semibold text-primary">
                    זהו סרטון הדגמה של התרגיל. משוב נשלח על סרטוני ביצוע שהמתאמן העלה.
                  </p>
                </article>
              ))}
            </div>
...
export function CoachDashboardPage({
  ...
  const selectExerciseForBuilder = useCallback(
    (
      exerciseId: string,
  ...
  const coachDemoVideoRecords = useMemo<CoachDemoVideo[]>(() => {
    ...
    for (const { client, details } of overviewRows) {
      ...
      for (const workout of details.workouts) {
        for (const item of workout.items) {
          ...
          for (const [sourceIndex, source] of exerciseDemoVideoSources(exercise).entries()) {
            records.push({
              id: `${client.client_id}-${workout.id}-${item.exerciseId}-${sourceIndex}-${source}`,
              clientName,
              workoutName: workout.name || "אימון",
              exerciseName: item.exerciseName || exercise?.name || "תרגיל",
              source,
            });
          }
        }
  ...
  }, [clientDetails, pendingCreatedExercise, selectExerciseForBuilder, selectedClientId]);
  ...
  }, [editingItemId, handleAddExerciseToDay, selectedClientId, showExerciseForm]);
  ...
  const savePlannedMenu = useCallback(async () => {
  ...
  }, [isCoach, isSelfSelected, savingPlannedMenu, selectedClientId]);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timeoutId);
  }, [isCoach, plannedMeals, savePlannedMenu, savingPlannedMenu, selectedClientId]);
        ...
        trackingLanding ? (
          <div className="flex items-center gap-1">
            ...
            >
              ...
              משובים
              {performanceVideoRecords.length + coachDemoVideoRecords.length > 0 ? (
                <span className="rounded-full bg-primary/15 px-1 py-0.5 text-[8px] leading-none">
                  {performanceVideoRecords.length + coachDemoVideoRecords.length}
                </span>
              ) : null}
            </button>
            <button
                                      ...
                                      >
                                        ...
                                        >
                                          <div className="coach-workout-header workout-builder-header flex flex-col gap-3 border-b border-border/60 pb-3 sm:flex-row sm:items-start sm:justify-between">
                                            ...
                                            <div className="flex w-full shrink-0 items-center justify-end gap-2 sm:w-auto">
                                              <button
                                                ...
                                                aria-label="פתיחת דוח"
                                                title="פתיחת דוח"
                                                aria-controls={`workout-report-${dayItem.id}`}
                                                aria-expanded={openWorkoutReportId === dayItem.id}
                                                className={`workout-report-trigger inline-flex h-9 items-center gap-1.5 rounded-xl border px-3 text-[11px] font-extrabold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
                                          ...
                                          <section
                                            ...
                                            data-open={openWorkoutReportId === dayItem.id}
                                            aria-hidden={openWorkoutReportId !== dayItem.id}
                                            aria-labelledby={`workout-report-heading-${dayItem.id}`}
                                          >
                                            <div className="workout-report-drawer-header">
                                              <div className="min-w-0">
                                                <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-primary">
                                                  דוח האימון
                                                </p>
                                              ...
                                              <button
                                                type="button"
                                                onClick={() => setOpenWorkoutReportId(null)}
                                                aria-label="סגירת דוח"
                                                className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-border bg-surface text-muted-foreground transition-colors hover:border-primary/40 hover:bg-primary/5 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                                              >
                                                <X className="h-4 w-4" aria-hidden="true" />
                                              </button>
                                            </div>
