...
Call log:
  - waiting for getByRole('textbox', { name: 'חיפוש לפי שם או אימייל' })
    - locator resolved to <input value="" type="text" id="coach-client-search" aria-label="חיפוש לפי שם או אימייל" placeholder="חיפוש לפי שם או אימייל..." data-tsd-source="/src/routes/coach.tsx:5524:17" class="w-full bg-transparent text-xs outline-none placeholder:text-muted-foreground"/>
    - fill("בדיקה")
...
> 292 |   await page.getByRole("textbox", { name: "חיפוש לפי שם או אימייל" }).fill("בדיקה");
  ...
  390 |   await expect(workoutNote).toBeHidden();
  391 | 
  392 |   await page.getByRole("button", { name: "סמן סט כבוצע" }).first().click();
```