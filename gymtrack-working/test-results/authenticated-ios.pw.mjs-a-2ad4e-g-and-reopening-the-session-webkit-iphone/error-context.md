...
Expected: "הערת סיום בטיוטת האימון"
Received: ""
Timeout:  8000ms
...
Call log:
  ...
  - waiting for getByPlaceholder('למשל: עומס קל במרפק ימין בסט האחרון...')
    19 × locator resolved to <textarea placeholder="למשל: עומס קל במרפק ימין בסט האחרון..." data-tsd-source="/src/routes/session.$workoutId.tsx:1601:15" class="w-full rounded-2xl border border-border p-2.5 text-xs outline-none focus:border-primary h-16"></textarea>
       - unexpected value ""

```

```yaml
- 'textbox "למשל: עומס קל במרפק ימין בסט האחרון..."'
```
